...
export default function QuickStartDetailsWizard({
  ...
  const saveStepData = async (updatedDraft: DraftData, stepNumber: number) => {
    ...
    // Auto-save to DB for existing profiles or invitation instances
    const effectiveInvId = newDraftState.invitationId || invitationId;
    if ((updatedDraft.id || effectiveInvId) && status === "authenticated") {
      try {
        const res = await fetch("/api/user/event-draft", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            isInvitationInstance: Boolean(effectiveInvId),
          }),
        });
        const data = await res.json();
        if (data.success && data.draft?.id) {
