  ...
  const eventsList =
    props.events && props.events.length > 0
      ? props.events.map((e, idx) => {
          ...
          return {
            title: e.title,
            time: `${e.date || props.weddingDate || "Celebration Day"} • ${e.time || "10:00 AM"}`,
            venue: venueName,
          ...
          {eventsList.map((evt, idx) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-[#d8b5c4] mb-4 leading-relaxed font-medium">
                  <span className="text-white font-semibold block mb-0.5">{evt.venue}</span>
                  {evt.address}
                </p>

                <div className="relative w-full h-36 rounded-lg overflow-hidden border border-[#e6c280]/30 mb-6 shadow-inner bg-[#1a0812]">
                  <iframe
