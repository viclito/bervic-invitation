...
export default function OrderOrCartModal({
  ...
  const effectiveMinCopies = Math.max(50, Number(linkedShopProduct?.minCopies) || 50);
  const currentCopies = customCopiesInput ? parseInt(customCopiesInput, 10) : Math.max(effectiveMinCopies, copies);

  const availablePresetCopies = useMemo(() => {
    const list = PRESET_COPIES.filter((c) => c >= effectiveMinCopies);
    if (!list.includes(effectiveMinCopies)) {
      list.unshift(effectiveMinCopies);
      list.sort((a, b) => a - b);
    }
    return list;
  }, [effectiveMinCopies]);
  ...
  const printingChargeResult = useMemo(() => {
    if (!printingChangeConfig.enabled) {
      return { fee: 0, breakdownText: "" };
    }
    return calculatePrintingChangeFee(currentCopies, printingChangeConfig);
  }, [printingChangeConfig, currentCopies]);
            ...
            <div className="space-y-6 animate-in fade-in duration-200">
              ...
              <div className="space-y-4 bg-white p-5 rounded-3xl border border-slate-200 shadow-2xs">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2.5">
                  {availablePresetCopies.map((preset) => {
                ...
                {/* Prominent Custom Quantity Card */}
                <div className="p-3.5 rounded-2xl bg-gradient-to-r from-amber-50/70 via-rose-50/40 to-white border-2 border-amber-300 shadow-2xs space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[11px] text-slate-600">
                        Need another quantity? Type your required print count here (Min. {effectiveMinCopies}):
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    <input
                      type="number"
                      min={effectiveMinCopies}
                      placeholder={`Min ${effectiveMinCopies} (e.g. 750)`}
                      value={customCopiesInput}
                      onChange={(e) => {
                        setCustomCopiesInput(e.target.value);
                        const val = parseInt(e.target.value, 10);
                        if (val >= effectiveMinCopies) {
                          clearFieldError("copies");
                        }
                      }}
                      className={`w-36 px-3 py-2 rounded-xl border-2 text-sm font-black text-slate-900 bg-white focus:outline-none transition-colors shadow-xs ${
                        formErrors.copies
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <strong className="text-[#991B1B] text-xs font-extrabold block truncate">
                      {isLinked
                        ? `₹${totalPrice + totalPrintingChangeFee} ${totalPrintingChangeFee > 0 ? `(Cards: ₹${totalPrice} + Print Charge: ₹${totalPrintingChangeFee})` : `(₹${unitPrice}/card)`}`
