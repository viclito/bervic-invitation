...
const CardTemplate27: React.FC<CardProps> = ({
  ...
  return (
    <div
      style={{ width: '1080px', height: '1080px', backgroundColor: '#F5E8D0', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px', boxSizing: 'border-box', color: '#4A3525' }}
    >
      <style>
        {`
      ...
      {/* Airmail Border */}
      <div
        style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, border: '25px solid transparent', borderImage: 'repeating-linear-gradient(-45deg, #c43a31, #c43a31 20px, transparent 20px, transparent 40px, #2a5a8a 40px, #2a5a8a 60px, transparent 60px, transparent 80px) 25', pointerEvents: 'none', zIndex: 1 }}
      />

      {/* Postmark Stamp */}
      <svg
        style={{ position: 'absolute', top: '60px', right: '60px', width: '180px', height: '180px', opacity: 0.6, zIndex: 2 }}
        viewBox="0 0 200 200"
      >
        ...
        <text style={{ fontSize: '30px', fontWeight: 600, fontFamily: "'Courier Prime', monospace", fill: '#c43a31', letterSpacing: '4px' }}>
          <textPath href="#curve" startOffset="50%" textAnchor="middle">AIR MAIL</textPath>
        </text>
        <text x="100" y="115" style={{ fontSize: '38px', fontWeight: 'bold', fontFamily: "'Special Elite', cursive", fill: '#c43a31' }} textAnchor="middle">
          LOVE
        </text>
      ...
      {/* Inner Content */}
      <div
        style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', zIndex: 3 }}
      >
        <div style={{ textAlign: 'center', marginTop: '40px' }}>
          ...
          <h1 style={{ fontFamily: "'Special Elite', cursive", fontSize: '84px', fontWeight: 700, margin: 0, lineHeight: 1.2 }}>
        ...
        <div style={{ position: 'relative', margin: '40px 0' }}>
          ...
          <img
            ...
            style={{ width: '460px', height: '340px', objectFit: 'cover', border: '8px solid #fff', boxShadow: '2px 4px 15px rgba(0,0,0,0.1)', filter: 'sepia(40%) contrast(110%)' }}
          />
        </div>

        <div style={{ textAlign: 'center', marginBottom: '40px', width: '100%', fontFamily: "'Courier Prime', monospace", fontSize: '34px', fontWeight: 600, lineHeight: 1.6 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '80%', margin: '0 auto 20px auto', borderTop: '1px solid #4A3525', borderBottom: '1px solid #4A3525', padding: '15px 0' }}>
            ...
            <span style={{ fontSize: '38px', fontWeight: 600 }}>✦</span>
            <span>{weddingTime}</span>
          </div>
          <div>{venue}</div>
          <div>{city}</div>
          <div style={{ marginTop: '20px', fontSize: '28px', fontWeight: 500, fontStyle: 'italic', letterSpacing: '2px' }}>
            {hashtag}
          </div>
        </div>
      </div>
